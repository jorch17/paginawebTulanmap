...
class Index:
    ...
    def GET(self):
        result = config.model.get_all_paradas().list() # get paradas table list
        for row in result:
            row.id_parada = config.make_secure_val(str(row.id_parada)) # apply HMAC to id_parada (primary key)
